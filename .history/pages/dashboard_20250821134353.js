...
export default function Dashboard() {
  ...
  // ✅ Componente reutilizable: Tarjeta de Curso
  const CourseCard = ({ course, isStudent = false, isTeacher = false }) => (
    <div
      key={course.id}
      className="border rounded-lg overflow-hidden hover:shadow-md transition flex flex-col"
      onClick={() =>
        router.push(
          isTeacher ? `/teacher/course/${course.id}` : `/courses/${course.id}`
        )
      }
    ...
    >
      ...
      <div className="h-36 bg-gray-200 relative">
        <img
          src={course.imageUrl || "/img/course-placeholder.jpg"}
          alt={course.title}
          ...
          <>
            ...
            <div className="flex items-center gap-1 mt-1">
              ...
              <span className="text-xs text-gray-600 ml-1">
                {course.averageRating !== "Sin" ? course.averageRating : "Sin"}{" "}
                ({course.reviewCount})
              </span>
            </div>
            ...
            {enrolledCourses.length > 0 ? (
              <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                {enrolledCourses.map((course) => (
                  <CourseCard key={course.id} course={course} isStudent />
                ))}
              </div>
            ) : (
