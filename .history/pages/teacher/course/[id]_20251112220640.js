    ...
    const loadEnrolledStudents = async () => {
      ...
      try {
        ...
        for (const docSnap of querySnapshot.docs) {
          ...
          // Validar que studentId exista y sea un string
          if (
            studentId &&
            ...
            if (studentDocSnap.exists()) {
              ...
              const studentToAdd = {
                id: studentId,
                name: studentData.name || studentData.email || "Sin nombre",
                email: studentData.email || "Sin email",
                enrollmentId: docSnap.id, // Para actualizar la calificación
                grade: enrollment.grade || null,
              };
              studentsList.push(studentToAdd);
              // Inicializar mapa de calificaciones
              gradesMap[studentId] = enrollment.grade || "";
            } else {
              // Opcional: manejar el caso donde el usuario no existe
              console.warn(
                `Usuario con ID ${studentId} no encontrado en la colección 'users'.`
              );
            }
          } else {
            console.warn(
              "studentId inválido o vacío encontrado en una inscripción:",
              docSnap.id
            );
          }
        }
