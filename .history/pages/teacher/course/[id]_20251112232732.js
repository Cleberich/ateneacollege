...
export default function CourseLessonsPage() {
  ...
  const [jitsiRoom, setJitsiRoom] = useState(""); // Campo para la sala Jitsi
  const [recordingUrl, setRecordingUrl] = useState(""); // Campo para la URL de la grabación
  ...
  // --- NUEVO: Para videoconferencia como profesor con JaaS ---
  const [isJitsiModalOpen, setIsJitsiModalOpen] = useState(false);
  const [currentLiveLesson, setCurrentLiveLesson] = useState(null);
  const [jitsiJwt, setJitsiJwt] = useState(null); // JWT para JaaS
  const [loadingJitsi, setLoadingJitsi] = useState(false); // Estado de carga para JaaS
  ...
  const handleAddLesson = async (e) => {
    ...
    try {
      ...
      if (type === "text") {
        ...
        if (!jitsiRoom.trim()) {
          setError("Por favor ingresa el nombre de la sala de Jitsi.");
          setAdding(false);
          return;
        }
        // --- CAMBIO CLAVE: Formato del roomName ---
        lessonContent = `${id}_${jitsiRoom.trim()}`; // Incluir courseId en el roomName
        if (recordingUrl.trim() && isValidUrl(recordingUrl)) {
          lessonRecordingUrl = recordingUrl.trim();
        }
      } else if (type === "quiz") {
      ...
      const newLesson = {
        ...
        title: title.trim(),
        type,
        content: lessonContent, // Este será el roomName con el formato `${courseId}_${jitsiRoom}`
        order: editingLesson
          ? editingLesson.order
      ...
      if (type === "live" && lessonRecordingUrl) {
        newLesson.recordingUrl = lessonRecordingUrl;
      }
  ...
  // --- NUEVO: Función para unirse a la sala como profesor con JaaS ---
  const joinLiveLessonAsTeacher = async (lesson) => {
    ...
    try {
      // 1. Llamar a tu API para generar el JWT
      const response = await fetch("/api/generateJitsiJwt", {
        ...
        body: JSON.stringify({
          roomName: lesson.content, // Usar el roomName completo (courseId_lessonId)
          role: "moderator",
          userInfo: {
            displayName: user?.displayName || user?.email || "Profesor",
            email: user?.email,
          },
        }),
      });
      ...
      if (!response.ok) {
        throw new Error("Error al generar el token de Jitsi");
      }

      const { jwt } = await response.json();
      setJitsiJwt(jwt);
    } catch (error) {
      console.error("Error obteniendo JWT para JaaS:", error);
      setError("No se pudo iniciar la reunión: " + error.message);
    ...
    <div className="min-h-screen p-10 grid grid-cols-1 lg:grid-cols-2 gap-8  mx-32">
      ...
      <div className="space-y-8">
        ...
        >
          ...
          {/* Contenido condicional para clase en vivo */}
          {type === "live" && (
            <div className="mb-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Nombre de la Sala de Jitsi *
                </label>
                <input
                  type="text"
                  value={jitsiRoom}
                  ...
                  required
                  className="w-full p-3 border border-gray-300 rounded-lg placeholder:text-black"
                  placeholder="nombre-de-la-sala-jitsi"
                />
                <p className="text-xs text-gray-500 mt-1">
                  El nombre de la sala donde se llevará a cabo la clase en vivo.
                  Se le agregará el ID del curso.
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  URL de la Grabación (Opcional)
                </label>
                <input
                  type="url"
                  value={recordingUrl}
                  ...
                  className="w-full p-3 border border-gray-300 rounded-lg placeholder:text-black"
                  placeholder="https://ejemplo.com/grabacion-clase.mp4"
                />
                <p className="text-xs text-gray-500 mt-1">
                  Si ya está disponible, pega aquí la URL de la grabación.
                </p>
              </div>
            </div>
          )}
        ...
        {/* Sección: Alumnos inscritos y calificaciones */}
        <div className="bg-white p-6 rounded-lg shadow">
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        {/* Visualización para clase en vivo */}
                        {lesson.type === "live" && (
                          <div className="mt-2">
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              Sala: {lesson.content}{" "}
                              {/* Mostrar el roomName completo */}
                            </p>
                            {/* Botón para unirse como profesor usando JaaS */}
                            <button
                              onClick={() => joinLiveLessonAsTeacher(lesson)}
                              className="block text-sm text-indigo-600 hover:underline mt-1"
                            ...
                            {lesson.recordingUrl && (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="block text-sm text-purple-600 hover:underline mt-1"
                              >
                                Ver Grabación
                              </a>
                            )}
      ...
      {/* Modal para JaaSMeeting */}
      {isJitsiModalOpen && currentLiveLesson && jitsiJwt && (
          ...
          <div className="bg-white rounded-lg shadow-lg max-w-6xl w-full max-h-[90vh] flex flex-col">
            <div className="p-5 border-b flex justify-between items-center">
              <h3 className="text-xl font-semibold text-gray-800">
                Clase en Vivo: {currentLiveLesson.title}
              </h3>
              <button
                onClick={() => {
                  setIsJitsiModalOpen(false);
                  setCurrentLiveLesson(null);
                  setJitsiJwt(null);
                }}
            ...
            <div className="p-5 flex-1 overflow-hidden">
              <JaaSMeeting
                appId={process.env.NEXT_PUBLIC_JAAS_APP_ID} // Tu appId de JaaS
                roomName={currentLiveLesson.content} // Nombre de la sala
                jwt={jitsiJwt} // JWT generado dinámicamente
                configOverwrite={{
                ...
                onApiReady={(externalApi) => {
                  console.log("API de JaaS lista", externalApi);
                }}
                onReadyToClose={() => {
                  console.log("Sala de JaaS lista para cerrar");
                  setIsJitsiModalOpen(false);
                  setCurrentLiveLesson(null);
                  setJitsiJwt(null);
                }}
      ...
      {/* Modal de carga */}
      {isJitsiModalOpen && loadingJitsi && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg p-6">
            <p>Conectando a la clase en vivo...</p>
          </div>
        </div>
      )}
    </div>
