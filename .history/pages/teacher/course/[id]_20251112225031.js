...
export default function CourseLessonsPage() {
  ...
  const [jitsiRoom, setJitsiRoom] = useState(""); // NUEVO: Campo para la sala Jitsi
  const [recordingUrl, setRecordingUrl] = useState(""); // NUEVO: Campo para la URL de la grabación
  ...
  // --- NUEVO: Para videoconferencia como profesor ---
  ...
  const handleAddLesson = async (e) => {
    ...
    try {
      let lessonContent = "";
      let lessonRecordingUrl = null; // NUEVO
      ...
      if (type === "text") {
        ...
        lessonContent = jitsiRoom.trim(); // El contenido será el nombre de la sala
        if (recordingUrl.trim() && isValidUrl(recordingUrl)) {
          lessonRecordingUrl = recordingUrl.trim(); // Opcional: guardar URL de grabación
        }
      } else if (type === "quiz") {
      ...
      // NUEVO: Agregar campo recordingUrl si es una clase en vivo
  ...
  // NUEVO: Función para unirse a la sala como profesor
  const joinLiveLessonAsTeacher = (lesson) => {
    setCurrentLiveLesson(lesson);
    setIsJitsiModalOpen(true);
  };

  // NUEVO: Cargar Jitsi API cuando se abre el modal
  useEffect(() => {
    if (isJitsiModalOpen && currentLiveLesson && jitsiContainerRef.current) {
      const loadJitsi = async () => {
        ...
        const waitForJitsi = () => {
          if (window.JitsiMeetExternalAPI) {
            ...
            const options = {
              roomName: currentLiveLesson.content, // Nombre de la sala de la lección
              width: "100%",
              height: "100%",
              ...
              configOverwrite: {
                startWithAudioMuted: false, // Micrófono encendido por defecto
                startWithVideoMuted: false, // Cámara encendida por defecto
            ...
            api.addEventListeners({
              videoConferenceJoined: () => {
                console.log("Profesor unido a la conferencia", currentLiveLesson.title);
              },
              videoConferenceLeft: () => {
                console.log("Profesor dejó la conferencia", currentLiveLesson.title);
              }
            });
          } else {
    ...
    // --- Limpiar Jitsi cuando se cierra el modal o se desmonta el componente ---
    return () => {
      if (jitsiApi) {
        jitsiApi.dispose();
        setJitsiApi(null);
  ...
  }, [isJitsiModalOpen, currentLiveLesson, user]);
  ...
  return (
        ...
        >
          ...
          <div className="mb-4">
            ...
            >
              ...
              <option value="live">Clase en Vivo</option> {/* NUEVO: Opción para clase en vivo */}
          ...
          {/* NUEVO: Contenido condicional para clase en vivo */}
          {type === "live" && (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        {/* NUEVO: Visualización para clase en vivo */}
                        {lesson.type === "live" && (
                          <div className="mt-2">
                            ...
                            <p className="text-sm text-gray-600 mt-1">
                              Sala: {lesson.content} {/* Nombre de la sala */}
                            </p>
                            {/* NUEVO: Botón para unirse como profesor */}
                            <button
    ...
    {/* NUEVO: Modal para Jitsi */}
    {isJitsiModalOpen && currentLiveLesson && (
