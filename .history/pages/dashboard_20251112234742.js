        ...
        if (data.role === "student") {
          ...
          const courses = await Promise.all(
            querySnapshot.docs.map(async (enrollmentDoc) => {
              ...
              if (courseDoc.exists()) {
                ...
                // Obtener la calificación de la inscripción
                ...
                return {
                  ...
                  grade: grade, // <-- Usamos la calificación de la inscripción
                  isEnrolled: true,
                };
              }
              return null;
          ...
          const allCoursesList = allCoursesSnapshot.docs.map((doc) => {
            ...
            return {
              id: doc.id,
              ...courseData,
              isEnrolled, // Marcamos si está inscrito
              progress: isEnrolled
                ? courses.find((c) => c.id === doc.id)?.progress || []
                : [],
              grade: isEnrolled
                ? courses.find((c) => c.id === doc.id)?.grade // <-- Obtenemos la calificación del curso inscrito
                : null,
            };
          });
                        ...
                        <div className="flex items-center mt-2 gap-1">
                          ...
                          <span
                            className={`font-bold text-lg ${
                              course.grade >= 6
                                ? "text-green-600"
                                : "text-red-600" // <-- Cambia color según aprobación
                            }`}
                          >
