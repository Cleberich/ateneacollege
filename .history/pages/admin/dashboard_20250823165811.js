...
export default function AdminDashboard() {
  ...
  const [reviews, setReviews] = useState([]); // Para calcular valoraciones
  ...
  // --- Estadísticas ---
  const totalStudents = [...new Set(enrollments.map((e) => e.userId))].length;
  const totalCourses = courses.length;

  // Curso con mejor valoración
  const courseRatings = courses.map((course) => {
    ...
    const avgRating =
      courseReviews.length > 0
        ...
        : 0;
    return { ...course, avgRating, reviewCount: courseReviews.length };
  });

  // Filtrar cursos con al menos 1 reseña y ordenar por promedio
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="mx-32 py-10 px-4">
        ...
        <p className="text-gray-600 mb-8">Hola, {admin?.name || "Admin"} 👋</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow text-center">
            ...
            {topRatedCourse ? (
              <>
                <p className="font-medium text-gray-800 mt-2 line-clamp-1">
                  {topRatedCourse.title}
                </p>
                <div className="flex justify-center mt-1">
                  <span className="text-yellow-500 text-sm">
                    {"⭐".repeat(Math.round(topRatedCourse.avgRating))}
                  </span>
                  <span className="text-sm text-gray-600 ml-1">
                    {topRatedCourse.avgRating.toFixed(1)}
                  </span>
                </div>
              </>
            ) : (
              <p className="text-gray-500 text-sm mt-2">Sin reseñas aún</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow mb-8">
          <h2 className="text-xl font-semibold mb-4">Buscar y Editar Cursos</h2>
          <input
            type="text"
        ...
        <main className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow overflow-y-auto max-h-[70vh]">
            ...
            <form onSubmit={handleAssignTeacherToCourse} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Profesor
                </label>
